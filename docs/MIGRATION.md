# NodeScope migration and v1 state format

nodescope v1 uses central state schema `1`.
...
1. nodescope reads the old state without modifying it.
...
```bash
nodescope host --cpu-alert 85 --ram-alert 90 --disk-alert 95 --metric-for 3m --unstable-after 20s --offline-after 90s
```

Later starts can simply use `nodescope host`; the stored policy is reused.
...
```bash
nodescope state backup --out pre-upgrade.bak
```
...
No state migration is required. NodeScope v1.1.0 keeps HTTP API `v1` and state schema `1`.

Recommended rollout:

1. Back up central state with `nodescope state backup`.
2. Replace the central binary with v1.1.0 and restart it.
3. Run `nodescope doctor` and `nodescope health`.
4. Upgrade agents when convenient; v1.0.0 agents remain compatible with the v1.1.0 central server.


## JJP -> NodeScope rename

The product name and primary CLI changed from `jjp` to `nodescope`. State schema `1` and HTTP API `v1` are unchanged. The default on-disk data directory intentionally remains the legacy `jjp` directory for compatibility, and `JJP_*` environment variables remain accepted as aliases for the new `NODESCOPE_*` names.
